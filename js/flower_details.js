...
if (product_quantity <= flower.stock) { fetch("https://flower-seal-backend.vercel.app/orders/create_order/", {
  method: "POST" , headers: { "Content-Type" : "application/json" , Authorization: `token ${token}`, }, body:
  JSON.stringify({ user_id: parseInt(userId), product_id: parseInt(flower.id), quantity: parseInt(product_quantity), }),
  }) .then((response)=> response.json())
  .then((response) => {
  ...
  <div class="card container bg-white text-dark index_flower_card" style="border-radius: 20px;">
    ...
    if (orderExists) {
    window.open(
    `https://flower-seal-backend.vercel.app/payment/payment/${flower.id}/`,
    "_blank"
    );
    } else {
    ...
    <div class="col-md-4 col-lg-6 mb-4">
      <div class="card bg-white text-dark p-3 index_flower_card" style="border-radius: 10px;">
        ...
        <div class="d-flex gap-3">
          <div>
            <a class="gradient-btn edit-comment" data-id="${comment.id}" data-name="${comment.name}"
